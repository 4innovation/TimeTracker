...
namespace DataGridViewRichTextBox
{
    ...
    public class RichTextBoxPrinter
    {
        ...
        public static Image Print(RichTextBox ctl, int width, int height, DataGridView dataGridView, int rowIndex, bool selected)
        {
            ...
            using (Graphics g = Graphics.FromImage(img))
            {
                ...
                try
                {
                    if (selected == false)
                    {
                        if (dataGridView.Rows[rowIndex].Cells["Status"].Value.ToString() == "Erledigt")
                        {
                            ctl.BackColor = Color.White;
                        }
                        else if (dataGridView.Rows[rowIndex].Cells["Datum"] != null && dataGridView.Rows[rowIndex].Cells["Termin"] != null)
                        {
                            if (dataGridView.Rows[rowIndex].Cells["Datum"].Value != null && dataGridView.Rows[rowIndex].Cells["Termin"].Value != null)
                            {
                                DateTime date = DateTime.Parse(dataGridView.Rows[rowIndex].Cells["Datum"].Value.ToString());
                                DateTime termin = new DateTime();
                                try
                                {
                                    termin = DateTime.Parse(dataGridView.Rows[rowIndex].Cells["Termin"].Value.ToString());

                                    if (DateTime.Now > termin)
                                    {
                                        ctl.BackColor = Color.OrangeRed;
                                    }
                                    else
                                    {
                                        ctl.BackColor = Color.White;
